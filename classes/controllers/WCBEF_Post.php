...
class WCBEF_Post
{
    ...
    private function __construct()
    {
        ...
        add_action('admin_post_wcbef_activation_plugin', [$this, 'activation_plugin']);
        add_action('admin_post_wcbef_deactivation_plugin', [$this, 'deactivation_plugin']);
    }
    ...
    public function activation_plugin()
    {
        ...
        if (isset($_POST['activation_type'])) {
            if ($_POST['activation_type'] == 'skip') {
                update_option('wcbef_is_active', 'skipped');
                ...
                if (!empty($_POST['email']) && !empty($_POST['industry'])) {
                    ...
                    $info = $activation_service->activation([
                        ...
                        'product_id' => 'wcbef',
                        'product_name' => WCBEF_LABEL,
                        'industry' => sanitize_text_field($_POST['industry']),
                        'multi_site' => is_multisite(),
                        'core_version' => null,
                        'subsystem_version' => WCBEF_VERSION,
                    ]);

                    if (!empty($info) && is_array($info)) {
                        if (!empty($info['result']) && $info['result'] == true) {
                            update_option('wcbef_is_active', 'yes');
                            $message = esc_html__('Success !', 'woocommerce-bulk-edit-free');
                        } else {
                            update_option('wcbef_is_active', 'no');
                            $message = (!empty($info['message'])) ? esc_html($info['message']) : esc_html__('System Error !', 'woocommerce-bulk-edit-free');
                        }
                    } else {
                        update_option('wcbef_is_active', 'no');
                        $message = esc_html__('Connection Timeout! Please Try Again', 'woocommerce-bulk-edit-free');
                    }
                }
            }
        }
